...
def init():
    """Initializes the model and class names for the Pressure model."""
    global model, class_names
    
    # Define the class names for the Pressure model in the correct order (index 0, 1)
    class_names = ['flat', 'full']
    
    # AZUREML_MODEL_DIR is an environment variable pointing to the model folder.
    model_root_path = os.getenv("AZUREML_MODEL_DIR")
    model_path = os.path.join(model_root_path, "best_cv_model")
...
def run(raw_data):
    """Processes an image and returns a prediction for tyre pressure."""
    try:
        ...
        # 3. Define and apply the image transformations for the Pressure model
        # Based on your dataloaders.py ('tfm_eval' for 'pressure').
